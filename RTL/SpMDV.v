...
(
	...
	// 2-MAC compute pipeline from W/P buffer
	reg [5:0] pipe_issue;   // 0~48, step by 2
	reg [5:0] pipe_done;    // 0~48, step by 2
	reg pipe_x_valid;
		...
		if (rst) begin
			...
			case (state)
				...
				COMPUTE_PIPE: begin
					...
					// Issue X reads from two duplicated vector SRAMs
					if (pipe_issue < 6'd48) begin
