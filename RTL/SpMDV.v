...
(
	...
	// pipeline registers
	reg [5:0] pipe_issue;   // issued W/P read count, 0~48
	reg [5:0] pipe_done;    // finished MAC count, 0~48

	reg pipe_wp_valid;      // W/P Q valid in this cycle
	reg pipe_x_valid;       // X Q valid in this cycle

	reg [1:0] pipe_wp_sram; // which W/P SRAM output is valid
	reg [1:0] pipe_bank;    // bank for vector index, 0~3

	reg signed [7:0] pipe_weight_r;

	wire [13:0] pipe_issue_index;
	wire [1:0] pipe_issue_sram;

	assign pipe_issue_index = ({5'd0, row_count} << 5) +
	                          ({5'd0, row_count} << 4) +
	                          {8'd0, pipe_issue};

	assign pipe_issue_sram = pipe_issue_index[13:12];

	reg signed [7:0] pipe_weight_q;
	reg [5:0] pipe_position_q;
	reg [7:0] pipe_vector_index;

	always @(*) begin
		case (pipe_wp_sram)
			2'd0: begin
				pipe_weight_q = w0_Q;
				pipe_position_q = p0_Q[5:0];
			end

			2'd1: begin
				pipe_weight_q = w1_Q;
				pipe_position_q = p1_Q[5:0];
			end

			2'd2: begin
				pipe_weight_q = w2_Q;
				pipe_position_q = p2_Q[5:0];
			end

			default: begin
				pipe_weight_q = 8'd0;
				pipe_position_q = 6'd0;
			end
		endcase
	end
	...
	always @(*) begin
		case (pipe_bank)
			2'd0: pipe_vector_index = 8'd0   + {2'd0, pipe_position_q};
			2'd1: pipe_vector_index = 8'd64  + {2'd0, pipe_position_q};
			2'd2: pipe_vector_index = 8'd128 + {2'd0, pipe_position_q};
			2'd3: pipe_vector_index = 8'd192 + {2'd0, pipe_position_q};
			default: pipe_vector_index = 8'd0;
		endcase
	end

	wire signed [15:0] pipe_product;
	wire signed [21:0] pipe_product_ext;

	assign pipe_product = $signed(pipe_weight_r) * $signed(x_Q);
	assign pipe_product_ext = {{6{pipe_product[15]}}, pipe_product};
	...
	always @(*) begin
		...
		case (state)
			...
			COMPUTE_PIPE: begin
				...
				// S0: issue next W/P read
				if (pipe_issue < 6'd48) begin
					if (pipe_issue_sram == 2'd0) begin
						w0_CEN = 1'b0;
						w0_WEN = 1'b1;
						w0_A = pipe_issue_index[11:0];

						p0_CEN = 1'b0;
						p0_WEN = 1'b1;
						p0_A = pipe_issue_index[11:0];
					end
					else if (pipe_issue_sram == 2'd1) begin
						w1_CEN = 1'b0;
						w1_WEN = 1'b1;
						w1_A = pipe_issue_index[11:0];

						p1_CEN = 1'b0;
						p1_WEN = 1'b1;
						p1_A = pipe_issue_index[11:0];
					end
					else begin
						w2_CEN = 1'b0;
						w2_WEN = 1'b1;
						w2_A = pipe_issue_index[11:0];

						p2_CEN = 1'b0;
						p2_WEN = 1'b1;
						p2_A = pipe_issue_index[11:0];
					end
				end

				// S1: W/P Q valid, issue vector read
				if (pipe_wp_valid) begin
					x_CEN = 1'b0;
					x_WEN = 1'b1;
					x_A = {vector_count, pipe_vector_index};
				end
			end
			...
			case (state)
				...
				COMPUTE_PIPE: begin
					...
					if (pipe_x_valid) begin
						sum <= sum + pipe_product_ext;

						if (pipe_done != 6'd48)
							pipe_done <= pipe_done + 6'd1;
					end


					// S1: W/P Q valid this cycle.
					// Vector read is issued by SRAM control block.
					// Latch weight for next cycle's MAC.
					if (pipe_wp_valid) begin
						pipe_weight_r <= pipe_weight_q;
					end

					pipe_x_valid <= pipe_wp_valid;


					// S0: issue next W/P read metadata
					if (pipe_issue < 6'd48) begin
						pipe_wp_valid <= 1'b1;
						pipe_wp_sram <= pipe_issue_sram;
						pipe_bank <= pipe_issue[1:0];
						pipe_issue <= pipe_issue + 6'd1;
					end
					else begin
						pipe_wp_valid <= 1'b0;
					end
	...
	always @(*) begin
		...
		case (state)
			...
			COMPUTE_PIPE: begin
				if (pipe_x_valid && pipe_done == 6'd47)
					nextstate = OUTPUT;
				else
					nextstate = COMPUTE_PIPE;
			end
